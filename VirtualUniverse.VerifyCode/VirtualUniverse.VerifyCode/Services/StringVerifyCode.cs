...
using System.Text;
using VirtualUniverse.Algorithm.MathComputer;

namespace VirtualUniverse.VerifyCode.Services
{
    /// <summary>
    /// 
    /// </summary>
    public class StringVerifyCode
    {
        ...
        public void SetInterferencePoint(int pointNumber = 20, int radius = 5, Brush brush = null, Color[] colors=null)
        {
            if (colors != null)
            {
                if (colors.Length == 0)
                {
                    throw new ArgumentException(nameof(colors) + "的值不能为空或长度不能小于等于0");
                }
                for (int i = 0; i < pointNumber; i++)
                {
                    var index = MathUtil.Random(0, colors.Length - 1);
        ...
        {
            var x = MathUtil.Random(0, ImageWidth);
            var y = MathUtil.Random(0, ImageHeight);
